...
class RangeImageSegmentor(EncoderDecoder3D):
    ...
    def predict(self,
        ...
        imgs = batch_inputs_dict['imgs']
        x = self.extract_feat(imgs)
        seg_labels_list, seg_logits_list_2d = self.decode_head.predict(
            x, batch_input_metas, self.test_cfg)

        return self.postprocess_result(seg_labels_list, seg_logits_list_2d, batch_data_samples)
    ...
    def postprocess_result(self, seg_labels_list: List[Tensor],
                           seg_logits_list_2d: List[Tensor],
        ...
        for i, (seg_pred, seg_logit) in enumerate(zip(seg_labels_list, seg_logits_list_2d)):
            batch_data_samples[i].set_data(
                {'pred_pts_seg': PointData(**{'pts_semantic_mask': seg_pred}),
                 'pred_semantic_logits': PointData(**{'semantic_logits': seg_logit})})
