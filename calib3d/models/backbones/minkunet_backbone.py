...
from calib3d.models.layers.minkowski_engine_block import (
    IS_MINKOWSKI_ENGINE_AVAILABLE, MinkowskiBasicBlock, MinkowskiBottleneck,
    MinkowskiConvModule)
from calib3d.models.layers.sparse_block import (SparseBasicBlock,
                                                SparseBottleneck,
                                                make_sparse_convmodule,
                                                replace_feature)
from calib3d.models.layers.spconv import IS_SPCONV2_AVAILABLE
from calib3d.models.layers.torchsparse import IS_TORCHSPARSE_AVAILABLE
from calib3d.models.layers.torchsparse_block import (TorchSparseBasicBlock,
...
class MinkUNetBackbone(BaseModule):
    ...
    def __init__(self,
        ...
        if sparseconv_backend == 'torchsparse':
            ...
            residual_branch = partial(TorchSparseConvModule, act_cfg=None)
        ...
        for i in range(num_stages):
            encoder_layer = [
                encoder_conv(
                    encoder_channels[i] if i == 0 else encoder_channels[i] * residual_block.expansion,
                    encoder_channels[i] if i == 0 else encoder_channels[i] * residual_block.expansion,
                    kernel_size=2,
                    stride=2,
            ...
            for j in range(encoder_blocks[i]):
                if j == 0 and encoder_channels[i] != encoder_channels[i + 1] * residual_block.expansion:
                    encoder_layer.append(
                        residual_block(
                            ...
                            downsample=residual_branch(
                                encoder_channels[i] if i == 0 else encoder_channels[i] * residual_block.expansion,
                                encoder_channels[i + 1] * residual_block.expansion,
                                kernel_size=1)
            ...
            decoder_layer = [
                decoder_conv(
                    decoder_channels[i] * residual_block.expansion,
            ...
            for j in range(decoder_blocks[i]):
                if j == 0:
                    decoder_layer.append(
                        residual_block(
                            decoder_channels[i + 1] + encoder_channels[-2 - i] if i == num_stages - 1 else decoder_channels[i + 1] + encoder_channels[-2 - i] * residual_block.expansion,
                            decoder_channels[i + 1],
                            downsample=residual_branch(
                                decoder_channels[i + 1] + encoder_channels[-2 - i] if i == num_stages - 1 else decoder_channels[i + 1] + encoder_channels[-2 - i] * residual_block.expansion,
                                decoder_channels[i + 1] * residual_block.expansion,
                                kernel_size=1)
                            if residual_branch is not None else None,
