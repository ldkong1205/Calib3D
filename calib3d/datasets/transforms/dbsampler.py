...
from calib3d.datasets.transforms import data_augment_utils
...
@TRANSFORMS.register_module()
class InstanceMixLabeled(BaseTransform):

    def __init__(self,
                 info_path: str,
                 data_root: str,
                 filter_by_min_points: Dict[str, int],
                 ...
                 backend_args: Optional[dict] = None) -> None:
        super(InstanceMixLabeled, self).__init__()
        self.data_root = data_root
        self.info_path = info_path
        self.points_loader = TRANSFORMS.build(points_loader)
        self.backend_args = backend_args
        ...
        with get_local_path(
                info_path, backend_args=backend_args) as local_path:
            db_infos = mmengine.load(open(local_path, 'rb'), file_format='pkl')
    ...
    @staticmethod
    def filter_by_min_points(db_infos: dict, min_gt_points_dict: Dict[str, int]) -> dict:
        for name, min_num in min_gt_points_dict.items():
            min_num = int(min_num)
            if min_num > 0:
                filtered_infos = []
                for info in db_infos[name]:
                    if info['num_points'] >= min_num:
                        filtered_infos.append(info)
                db_infos[name] = filtered_infos
        return db_infos
    ...
    def transform(self, input_dict: dict) -> dict:
        s_points_list = []
        s_pts_semantic_mask_list = []
        for class_name, max_sample_num in zip(self.sample_classes,
                                              self.sample_max_nums):
            class_label = self.class_mapping[class_name]
            sampled_num = np.random.randint(0, max_sample_num)
            if sampled_num > 0:
                ...
                for info in sampled:
                    file_path = os.path.join(
                        ...
                        info['path']) if self.data_root else info['path']
                    results = dict(lidar_points=dict(lidar_path=file_path))
                    results = self.points_loader(results)
                    if self.pre_transform is not None:
                        results = self.pre_transform(results)
                    s_points = results['points']
                    s_pts_semantic_mask = np.ones((s_points.shape[0],), dtype=np.int64) * class_label
                    s_points_list.append(s_points)
                    s_pts_semantic_mask_list.append(s_pts_semantic_mask)
        points = input_dict['points']
        pts_semantic_mask = input_dict['pts_semantic_mask']
        if len(s_points_list) > 0:
            sampled_points = s_points_list[0].cat(s_points_list)
            sampled_pts_semantic_mask = np.concatenate(s_pts_semantic_mask_list)
            points = points.cat([points, sampled_points])
            pts_semantic_mask = np.concatenate([pts_semantic_mask, sampled_pts_semantic_mask])
            if 'image_features' in input_dict:
                image_features = input_dict['image_features']
                sampled_num = pts_semantic_mask.shape[0] - image_features.shape[0]
                sampled_image_features = torch.zeros((sampled_num, image_features.shape[-1]), dtype=torch.float32)
                image_features = torch.cat((image_features, sampled_image_features), dim=0)
                input_dict['image_features'] = image_features
