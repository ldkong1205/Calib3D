...
from calib3d.models.task_modules import VoxelGenerator
from calib3d.registry import TRANSFORMS
from calib3d.structures import (CameraInstance3DBoxes, DepthInstance3DBoxes,
...
class LaserMix(BaseTransform):
    ...
    def laser_mix_transform(self, input_dict: dict, mix_results: dict) -> dict:
        ...
        if 'image_features' in input_dict:
            image_features = input_dict['image_features']
            mix_image_features = mix_results['image_features']
            out_image_features = []

        if 'image_logits' in input_dict:
            image_logits = input_dict['image_logits']
            mix_image_logits = mix_results['image_logits']
            out_image_logits = []
        ...
        for i in range(num_areas):
            ...
            if i % 2 == 0:  # pick from original point cloud
                ...
                if 'image_features' in input_dict:
                    out_image_features.append(image_features[idx])
                if 'image_logits' in input_dict:
                    out_image_logits.append(image_logits[idx])
                ...
                if 'image_features' in input_dict:
                    out_image_features.append(mix_image_features[idx])
                if 'image_logits' in input_dict:
                    out_image_logits.append(mix_image_logits[idx])
        ...
        if 'image_features' in input_dict:
            input_dict['image_features'] = torch.cat(out_image_features, dim=0)
        if 'image_logits' in input_dict:
            input_dict['image_logits'] = torch.cat(out_image_logits, dim=0)
        return input_dict
...
class FrustumMix(BaseTransform):
    ...
    def frustum_horizontal_mix_transform(self, input_dict: dict,
        ...
        num_areas = np.random.choice(self.num_areas, size=1)[0]
        beam_list = np.linspace(0, self.H, num_areas + 1, dtype=np.int64)
        out_points = []
        out_pts_semantic_mask = []
        for i in range(num_areas):
            start_beam = beam_list[i]
            end_beam = beam_list[i + 1]
            if i % 2 == 0:
                idx = (coors >= start_beam) & (coors < end_beam)
                out_points.append(points[idx])
                out_pts_semantic_mask.append(pts_semantic_mask[idx.numpy()])
            else:
                idx = (mix_coors >= start_beam) & (mix_coors < end_beam)
                out_points.append(mix_points[idx])
                out_pts_semantic_mask.append(
                    mix_pts_semantic_mask[idx.numpy()])
        out_points = points.cat(out_points)
    ...
    def frustum_vertical_mix_transform(self, input_dict: dict,
        ...
        start = np.random.randint(0, self.W // 2)
        end = start + self.W // 2

        idx = (coors < start) | (coors >= end)
        mix_idx = (mix_coors >= start) & (mix_coors < end)
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if self.pre_transform is not None:
            # pre_transform may also require dataset
            mix_results.update({'dataset': dataset})
            # before frustummix need to go through
            # the necessary pre_transform
            mix_results = self.pre_transform(mix_results)
            mix_results.pop('dataset')
...
@TRANSFORMS.register_module()
class SemkittiSamFeature(BaseTransform):

    def __init__(self, sam_feature_root: str) -> None:
        self.sam_feature_root = sam_feature_root

    def transform(self, input_dict: dict) -> dict:
        assert 'images' in input_dict
        path_splits = input_dict['lidar_path'].split('/')
        h, w = input_dict['images']['CAM2']['height'], input_dict['images']['CAM2']['width']
        lidar2img = torch.tensor(input_dict['images']['CAM2']['lidar2img'], dtype=torch.float32)
        points_3d = input_dict['points'].coord
        points_2d = self.point_lidar2img(points_3d, lidar2img)
        image_sam_features = torch.zeros((points_3d.shape[0], 256), dtype=torch.float32)
        keep_idx = points_3d[:, 0] > 0  # only keep point in front of the vehicle
        keep_frustum = ((points_2d[:, 0] > 0) &
                        (points_2d[:, 1] > 0) &
                        (points_2d[:, 0] < w) &
                        (points_2d[:, 1] < h))
        keep_idx = keep_idx & keep_frustum
        matching_pixel = points_2d[keep_idx]
        matching_pixel = self.apply_coords(matching_pixel, (h, w))
        sam_feature_path = osp.join(self.sam_feature_root, path_splits[-3], path_splits[-1])
        if sam_feature_path.startswith('s3://'):
            assert client is not None
            sam_feature = torch.tensor(np.frombuffer(client.get(sam_feature_path), dtype=np.float32).reshape(1, 256, 64, 64))
        else:
            sam_feature = torch.tensor(np.fromfile(sam_feature_path, dtype=np.float32).reshape(1, 256, 64, 64))
        grid = (matching_pixel / 1024) * 2 - 1
        grid = grid.unsqueeze(0).unsqueeze(0)
        sam_feature = F.grid_sample(sam_feature, grid, align_corners=False).squeeze().transpose(0, 1)
        pairing_points = torch.where(keep_idx == True)[0]
        image_sam_features[pairing_points] = sam_feature
        input_dict['image_features'] = image_sam_features
        return input_dict

    def point_lidar2img(self, points_3d: Tensor, proj_mat: Tensor) -> Tensor:
    ...
    def get_preprocess_shape(self, old_h: int, old_w: int, long_side_length: int) -> Tuple[int, int]:
    ...
    def apply_coords(self, coords: Tensor, original_size: Tuple[int, int]) -> Tensor:
        ...
        new_h, new_w = self.get_preprocess_shape(original_size[0], original_size[1], 1024)
...
@TRANSFORMS.register_module()
class SemkittiLogit(BaseTransform):

    def __init__(self, logits_root: str) -> None:
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        lidar2img = torch.tensor(input_dict['images']['CAM2']['lidar2img'], dtype=torch.float32)
        # lidar2img = np.array(input_dict['images']['CAM2']['lidar2img'], dtype=np.float32)

        points_3d = input_dict['points'].coord
        # points_3d = input_dict['points'].coord.numpy()

        points_2d = self.point_lidar2img(points_3d, lidar2img)

        image_logits_features = torch.zeros((points_3d.shape[0], 19), dtype=torch.float32)
        # image_logits_features = np.zeros((points_3d.shape[0], 19), dtype=np.float32)
        ...
        keep_frustum = ((points_2d[:, 0] > 0) &
                        ...
                        (points_2d[:, 1] < h))
        keep_idx = keep_idx & keep_frustum
        logits_path = osp.join(self.logits_root, path_splits[-3], path_splits[-1])

        if logits_path.startswith('s3://'):
            assert client is not None
            logits = torch.tensor(np.frombuffer(client.get(logits_path), dtype=np.float32).reshape(-1, 19))
            # logits = np.frombuffer(client.get(logits_path), dtype=np.float32).reshape(-1, 19)
        else:
            logits = torch.tensor(np.fromfile(logits_path, dtype=np.float32).reshape(-1, 19))
            # logits = np.fromfile(logits_path, dtype=np.float32).reshape(-1, 19)
        
        
        # check if the shapes are mismatched
        if logits.shape[0] != image_logits_features[keep_idx].shape[0]:

            if logits.shape[0] > image_logits_features[keep_idx].shape[0]:
                l = logits.shape[0] - image_logits_features[keep_idx].shape[0]
                logits = logits[:-l, :]
            elif logits.shape[0] < image_logits_features[keep_idx].shape[0]:
                l = image_logits_features[keep_idx].shape[0] - logits.shape[0]
                # find indices where keep_idx is True
                true_indices = np.where(keep_idx)[0]
                # set the last 'l' indices of keep_idx to False
                keep_idx[true_indices[-l:]] = False
        

        image_logits_features[keep_idx] = logits
        # image_logits_features[keep_idx, :] = logits
        # image_logits_features = torch.from_numpy(image_logits_features)

        input_dict['image_logits'] = image_logits_features
    ...
    def point_lidar2img_numpy(self, points_3d: np.ndarray, proj_mat: np.ndarray) -> np.ndarray:
        # add an additional ones column to the points_3d array for homogeneous coordinates
        points_4 = np.hstack([points_3d, np.ones((points_3d.shape[0], 1), dtype=points_3d.dtype)])

        # perform the matrix multiplication
        point_2d = np.dot(points_4, proj_mat.T)

        # divide by the last coordinate to get normalized 2D coordinates
        points_2d_res = point_2d[:, :2] / point_2d[:, 2:3]

        return points_2d_res


@TRANSFORMS.register_module()
class NusSamFeature(BaseTransform):

    def __init__(self, sam_feature_root: str, min_dist: float = 0.0) -> None:
        self.sam_feature_root = sam_feature_root
        self.min_dist = min_dist

    def transform(self, input_dict: dict) -> dict:
        assert 'images' in input_dict
        pc_original = LidarPointCloud.from_file(input_dict['lidar_path'])
        image_sam_features = torch.zeros((pc_original.points.shape[1], 256), dtype=torch.float32)
        for cam in input_dict['images']:
            h, w = input_dict['images'][cam]['height'], input_dict['images'][cam]['width']
            pc = copy.deepcopy(pc_original)

            pc.rotate(input_dict['lidar2ego_rotation'])
            pc.translate(input_dict['lidar2ego_translation'])

            pc.rotate(input_dict['ego2global_rotation'])
            pc.translate(input_dict['ego2global_translation'])

            pc.translate(-input_dict['images'][cam]['ego2global_translation'])
            pc.rotate(input_dict['images'][cam]['ego2global_rotation'].T)

            pc.translate(-input_dict['images'][cam]['sensor2ego_translation'])
            pc.rotate(input_dict['images'][cam]['sensor2ego_rotation'].T)

            depths = pc.points[2, :]

            points = view_points(pc.points[:3, :],
                                 input_dict['images'][cam]['cam_intrinsic'],
                                 normalize=True)

            points = points[:2].T
            keep_idx = ((depths > self.min_dist) &
                        (points[:, 0] > 0) &
                        (points[:, 1] > 0) &
                        (points[:, 0] < w - 1) &
                        (points[:, 1] < h - 1))
            matching_pixel = points[keep_idx]
            matching_pixel = self.apply_coords(matching_pixel, (h, w))
            sam_feature_path = osp.join(self.sam_feature_root, input_dict['images'][cam]['sample_data_token'] + '.bin')
            if sam_feature_path.startswith('s3://'):
                assert client is not None
                sam_feature = torch.tensor(np.frombuffer(client.get(sam_feature_path), dtype=np.float32).reshape(1, 256, 64, 64))
            else:
...
@TRANSFORMS.register_module()
class NusLogit(BaseTransform):

    def __init__(self, logits_root: str, min_dist: float = 0.0) -> None:
        self.logits_root = logits_root
        self.min_dist = min_dist
    ...
    def transform(self, input_dict: dict) -> dict:
        assert 'images' in input_dict
        pc_original = LidarPointCloud.from_file(input_dict['lidar_path'])
        image_logits_features = torch.zeros((pc_original.points.shape[1], 16), dtype=torch.float32)
        for cam in input_dict['images']:
            h, w = input_dict['images'][cam]['height'], input_dict['images'][cam]['width']
            pc = copy.deepcopy(pc_original)
            ...
            keep_idx = ((depths > self.min_dist) &
                        ...
                        (points[:, 0] < w - 1) &
                        (points[:, 1] < h - 1))
            logits_path = osp.join(self.logits_root, input_dict['images'][cam]['sample_data_token'] + '.bin')
            if logits_path.startswith('s3://'):
                assert client is not None
                logits = torch.tensor(np.frombuffer(client.get(logits_path), dtype=np.float32).reshape(-1, 16))
            else:
                logits = torch.tensor(np.fromfile(logits_path, dtype=np.float32).reshape(-1, 16))
